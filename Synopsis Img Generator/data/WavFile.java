...
public class WavFile {
	...
	public WavFile(final File file) throws UnsupportedAudioFileException, IOException {
		...
		final long dataLength = framesCount * af.getSampleSizeInBits() * af.getChannels() / 8;

		data = new byte[(int) dataLength];
		ais.read(data);

		final AudioInputStream aisForPlay = AudioSystem.getAudioInputStream(file);
		try {
	 ...
	 */
	public int getSampleInt(final int sampleNumber) {
		...
		final int sample = ByteBuffer.wrap(sampleBytes).order(ByteOrder.LITTLE_ENDIAN).getInt();
		return sample;
	}
	...
	public static void main(final String[] args) throws UnsupportedAudioFileException, IOException {
		...
		for (int i = 0; i < wav.getFramesCount(); i++) {
			final int amplitude = wav.getSampleInt(i);
